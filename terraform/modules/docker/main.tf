...
resource "null_resource" "dbt_trino_pkg" {
      provisioner "local-exec" {
        command = <<EOF
        docker login -u _json_key_base64 -p ${var.registry-access-key} https://${var.registry-location}-docker.pkg.dev
        docker build -t ${var.registry-location}-docker.pkg.dev/${var.gcp-project-id}/${var.data-services-repository-id}/${var.dbt-trino-pkg-image-name}:${var.dbt-trino-pkg-image-tag} ${var.dbt-trino-pkg-docker-file-location}
        docker push ${var.registry-location}-docker.pkg.dev/${var.gcp-project-id}/${var.data-services-repository-id}/${var.dbt-trino-pkg-image-name}:${var.dbt-trino-pkg-image-tag}
        EOF
      }
}